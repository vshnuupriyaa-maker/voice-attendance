...
<div class="container">
  ...
  <div class="panel">
    ...
    <button class="btn btn-mic btn-large" id="reg-record-btn" onclick="startRecording()">Start Recording</button>
...
async function startRecording() {
  ...
  try {
    ...
    regMediaRecorder.ondataavailable = (e) => {
      if (e.data.size > 0) regAudioChunks.push(e.data);
    };
    ...
    document.getElementById("reg-record-btn").onclick = stopRecording;
    setTimeout(stopRecording, 5000);
  } catch(err) {
    if (err.name === "NotAllowedError") {
      alert("Microphone blocked! Please allow microphone in browser settings.");
    } else {
...
function stopRecording() {
  if (regMediaRecorder && regMediaRecorder.state !== "inactive") {
    ...
    document.getElementById("reg-record-btn").textContent = "Record Again";
    document.getElementById("reg-record-btn").onclick = startRecording;
  }
}
...
async function saveRegistration() {
  ...
  try {
    const res = await fetch("/api/register-student", {
      ...
      body: JSON.stringify({ name: name, audio: audioB64, spoken_text: regSpokenText })
    });
    const data = await res.json();
    const responseBox = document.getElementById("reg-response");
    if (data.success) {
      responseBox.textContent = "Registered successfully!";
